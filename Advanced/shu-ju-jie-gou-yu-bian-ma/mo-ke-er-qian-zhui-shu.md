...
当一个节点在另一个节点中被引用时，包含的是 `H(rlp.encode(x))`，其中 `H(x) = keccak256(x) if len(x) > > = 32 else x` 和 `rlp.encode` 是[递归长度前缀](di-gui-chang-du-qian-zhui-bian-ma-rlp.md)编码函数。
...
#### 交易树 <a href="#transaction-trie" id="transaction-trie"></a>
